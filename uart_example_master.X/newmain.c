...
#pragma config FOSC = XT   // Osilatör Seçimi bitleri (RC osilatörü)
...
{
    BRGH = 1;   // Yüksek Baud Hızı Seçimi biti (4MHz kristal ile 9600bps)
    SPBRG = 25; // Baud Hızı Oluşturucu değeri (4MHz kristal ile 9600bps)
    SYNC = 0;   // Asenkron mod
...
void main()
{
    TRISB = 0xFF; // PORTB'yi giriş olarak ayarla
    nRBPU = 0;    // PortB pull-up'ları etkinleştir
    UART_basla(); // UART iletişimini başlat
    while (1)
    {
        UART_yaz(PORTB); // PORTB'nin değerini UART üzerinden gönder
